...
public interface GatewayParameterChannelRepo extends JpaRepository<GatewayParameterChannel, GatewayParameterChannelId> {
    ...
    @Query("select km from GATEWAY_PARAMETER_CHANNEL km where km.transCode = ?1 and km.systemIdOrMcpId = ?2")
    GatewayParameterChannel findByTransCodeAndSystemIdOrMcpId(String transCode, String systemIdOrMcpId);
}
